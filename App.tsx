    ...
    } catch (e)
{
  ...
  const handleGenerateAnalysis = async (data: EvaluationData) => {
    if (!apiKey) {
      showNotification('الرجاء إعداد مفتاح Gemini API أولاً.', 'error');
    ...
    try {
      ...
      console.error(error);
      setAnalysisError('حدث خطأ أثناء إنشاء التحليل. تأكد من صحة مفتاح API الخاص بك وحاول مرة أخرى.');
    } finally {
  ...
  return (
    <div className="bg-slate-50 min-h-screen" style={{ backgroundColor: theme.primaryHex + '10' }}>
      <Header theme={theme} setTheme={setTheme} onOpenApiKeySettings={() => setIsApiKeyModalOpen(true)} />
